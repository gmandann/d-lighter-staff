...
//time keepers
unsigned long readLastSerialTimer = 0;   // the last time we processed doBlink()
unsigned long serialPing = 10000; // milliseconds between doBlink() calls
unsigned long readLastConnectionStatus = 0;   // the last time we processed doBlink()
unsigned long connectionPing = 20000; // milliseconds between doBlink() calls
unsigned long connectionDisconnectTime = 30000;
unsigned long timeKeeper = 0;    // a millis() time-slice

//// keep track of staffs /////
bool present01 = false;
bool present02 = false;
bool present03 = false;
bool present04 = false;
bool present05 = false;
bool present06 = false;
bool present07 = false;
bool present08 = false;
bool present09 = false;

//track time since last checkin by particular staff
unsigned long time01 = 0;
unsigned long time02 = 0;
unsigned long time03 = 0;
unsigned long time04 = 0;
unsigned long time05 = 0;
unsigned long time06 = 0;
unsigned long time07 = 0;
unsigned long time08 = 0;
unsigned long time09 = 0;

/// keep track of number of staffs present
int numberOfStaffsPresent = 0;
...
void setup()
{
  ...
  LEDS.addLeds<LPD8806, 3, 2>(leds, NUM_LEDS);
...
//		}
  ...
  if (readLastSerialTimer + serialPing < timeKeeper){
    readLastSerialTimer = timeKeeper;
    readSerialData();
  }
  if (readLastConnectionStatus + connectionPing < timeKeeper){
    readLastConnectionStatus = timeKeeper;
    checkLostConnections();
  
  } 
  
  if(numberOfStaffsPresent > 7){
    Serial.println("we've got 7!!!");
    int q = 30;
    for(int i=0; i<q*20; i++) {rainbow_loop(10, 20);}
  } 
  if(present02 == true){
  }
  if(present03 == true){
...
void readSerialData(){
  Serial.println("readSerialData");
   // Read all serial data available, as fast as possible looking for terminators
...
void checkStatus(char * staffName){
  if(strcmp(staffName, "01")==0){
    present01 = true;
    time01 = timeKeeper;
    Serial.println("01 present");
  }
  else if(strcmp(staffName, "02")==0){
    present02 = true;
    time02 = timeKeeper;
    Serial.println("02 present");
  }
  else if(strcmp(staffName, "03")==0){
    present03 = true;
    time03 = timeKeeper;
    Serial.println("03 present");
  }
  else if(strcmp(staffName, "04")==0){
    present04 = true;
    time04 = timeKeeper;
    Serial.println("04 present");
  }
  else if(strcmp(staffName, "05")==0){
    present05 = true;
    time05 = timeKeeper;
    Serial.println("05 present");
  }
  else if(strcmp(staffName, "06")==0){
    present06 = true;
    time06 = timeKeeper;
    Serial.println("06 present");
  }
  if(strcmp(staffName, "07")==0){
    present07 = true;
    time07 = timeKeeper;
    Serial.println("07 present");
  }
  else if(strcmp(staffName, "08")==0){
    present08 = true;
    time08 = timeKeeper;
    Serial.println("08 present");
  }
  else if(strcmp(staffName, "09")==0){
    present09 = true;
    time09 = timeKeeper;
    Serial.println("09 present");
  }
  else{
...
void checkLostConnections(){
  Serial.println("checkLostConnections");
        countStaffs();
        Serial.println("StaffCount");
        Serial.println(numberOfStaffsPresent);
  
        if (timeKeeper - time01 > connectionDisconnectTime){
          present01 = false;
          Serial.println("01 not present");
        }
        if (timeKeeper - time02 > connectionDisconnectTime){
              present02 = false;
              Serial.println("02 not present");
        }
        if (timeKeeper - time03 > connectionDisconnectTime){
              present03 = false;
              Serial.println("03 not present");
        }
        if (timeKeeper - time04 > connectionDisconnectTime){
          present04 = false;
          Serial.println("04 not present");
        }
        if (timeKeeper - time05 > connectionDisconnectTime){
              present05 = false;
              Serial.println("05 not present");
        }
        if (timeKeeper - time06 > connectionDisconnectTime){
              present06 = false;
              Serial.println("06 not present");
        }
        if (timeKeeper - time07 > connectionDisconnectTime){
          present07 = false;
          Serial.println("07 not present");
        }
        if (timeKeeper - time08 > connectionDisconnectTime){
              present08 = false;
              Serial.println("08 not present");
        }
        if (timeKeeper - time09 > connectionDisconnectTime){
              present09 = false;
              Serial.println("09 not present");
        }
...
void countStaffs(){
  numberOfStaffsPresent = present01 + present02 + present03 + present04 + present05 + present06 + present07 + present08 + present09;
