...
unsigned long lastBroadcastName = 0;
unsigned long broadcastNamePing = 20000;
...
void setup()
{
  Serial1.begin(9600);
  Serial.begin(9600);
  LEDS.setBrightness(12); // SET BRIGHTNESS TO 1/2 POWER
  LEDS.addLeds<LPD8806, 3, 2>(leds, NUM_LEDS);
  sCmd.addCommand("m",   set_mode_strip);
  sCmd.setDefaultHandler(unrecognized);      // Handler for command that isn't matched  (says "What?")
  
  for (int i=0; i < numberOfStaffsPossible; i++){
      staffManager[i][0]=i;
    }
...
//  demo_mode();
  if (lastBroadcastName + broadcastNamePing < timeKeeper){
    lastBroadcastName = timeKeeper;
    broadcastName();
  }
 ...
 for(int i = 0 ; i < numberOfStaffsPossible; i++ ) {
   if(staffManager[i][1 == 1]){
      set_color_led(i, 255, 255, 255);
    }
    else{
...
void readSerialData(){
  ...
  while(Serial1.available() > 0)
  {
    ...
    {
       ended = true;
       int staffInteger = atoi(inData);
       checkStatus(staffInteger);
    }
    else
    {
      if(index < 5)
...
void broadcastName(){
  Serial1.write(thisStaffName);
}


void checkStatus(int staffName){
  
  for (int i=0; i < numberOfStaffsPossible; i++){
      
    if(staffName == staffManager[i][0]){
      staffManager[i][1]=1;
      staffManager[i][2] = timeKeeper;
      Serial.print("staff ") + Serial.print(staffManager[i][0]) + Serial.println(" present");
    }
    else{
...
void checkLostConnections(){
  Serial.println("checkLostConnections");
        ...
        for (int i=0; i < numberOfStaffsPossible; i++){
          ...
          if(timeKeeper - staffManager[i][2]  > connectionDisconnectTime){
             staffManager[i][1] = 0;
             Serial.print("staff ") + Serial.print(staffManager[i][0]) + Serial.println("not present");
...
void countStaffs(){
  ...
  for (int i=0; i < numberOfStaffsPossible; i++){
          ...
          if(staffManager[i][1] == 1){
            newStaffCount = newStaffCount + 1 ;
          }
          
        numberOfStaffsPresent = newStaffCount;
...
void pulse_staff_locator(int ahue, int idelay) { //-PULSE SATURATION ON ALL LEDS TO ONE COLOR 
  ...
  HSVtoRGB(ahue, isat, 255, acolor);
    ...
    for(int i = 0 ; i < numberOfStaffsPresent; i++ ) {
      set_color_led(i, acolor[0], acolor[1], acolor[2]);
    }
    LEDS.show();
    delay(idelay);
